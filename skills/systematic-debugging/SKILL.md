...
1. **Reproduce with a failing test first.** Before changing production code, write the smallest test that reproduces the bug and watch it fail for the bug's reason. You MAY proceed without a reproducing test ONLY when reproduction is genuinely impossible after real effort (a confirmed environmental, timing-dependent, or external cause — see "When Investigation Finds No Code Root Cause"), and only after you (1) summarize the complete reasons, (2) obtain the user's explicit permission, and (3) record those reasons in a comment in the relevant production code.
2. **Apply a single fix at the root cause.** One change, addressing the cause you identified. You MUST NOT bundle unrelated refactors or "while I'm here" improvements. The root-cause fix is in and verified AND the bug was an invalid value reaching a dangerous operation → you SHOULD additionally harden each layer that value crosses, so the class of bug becomes structurally impossible; read `@defense-in-depth.md` and apply it.
...
4. **Sweep for the same root cause.** The fix is verified → you MUST search the codebase for every other site sharing this root cause — the same call pattern, the same missing guard, the same wrong assumption, the same copied block — and you MUST state which searches you ran. A site that has the defect MUST be fixed in this change; a site that rests on the same assumption but cannot fail today MUST be made safe or recorded as follow-up. The same root cause is the same bug, not the "while I'm here" bundling step 2 forbids. The sweep is bounded by the cause: you MUST run only the searches the root cause defines, and you MUST NOT audit unrelated code.
5. **Ask the prevention question — after the fix lands.** What would have made this bug impossible or caught it earlier? A cheap hardening (a validation, an assertion, a lint rule) MAY follow as its own change; an architectural answer goes to the user via the `confirming-with-the-user` skill. You MUST NOT bundle the prevention work into the fix itself.
6. **The fix failed → STOP and count.** Fewer than 3 fixes tried → return to Phase 1 with the new information. **3 or more tried → you MUST stop fixing and question the architecture** (below).
...
A thorough investigation shows the cause is genuinely environmental, timing-dependent, or external → you MUST: document what you investigated; implement appropriate handling (retry / timeout / a clear error); and add logging for future diagnosis. This is the only path on which Phase 4's reproducing test may be skipped, and only through that step's escape clause (summarize the reasons, obtain the user's explicit permission, record them in a code comment). You MUST first be sure you actually finished Phase 1.
