...
- ADRs live in `docs/adr/` at the host project root, or the location the user confirmed for this project.
- Name each file with a sequential number and a slug: `0001-event-sourced-orders.md`, `0002-postgres-for-write-model.md`.
- Numbering a new ADR → scan `docs/adr/` for the highest existing number and increment by one.
- Create the directory only together with the first ADR.
...
You MUST NOT add sections, headers, or fields beyond this template unless they carry real content (below).
...
- **Status** (`accepted | superseded by ADR-NNNN`) — MAY be added when a decision is actually revisited, not speculatively.
...
- **Explicit NO-decisions.** "We will not X because Y." Record a rejection whenever the rejection is non-obvious.
