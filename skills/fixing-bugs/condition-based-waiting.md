...
Load this technique during `fixing-bugs` when the failure is a **flaky or timing-dependent test** — one that passes sometimes and fails under load, in parallel, or in CI.
