...
You MUST trace backward through the call chain to the original trigger and fix there. You MUST NOT fix only where the error appears.

## The tracing process

1. **Observe the symptom precisely.** Capture the exact failing operation and its inputs.
...
The chain is too long or dynamic to follow by reading code → add temporary instrumentation at the point of the dangerous operation, then run once and read the evidence:
...
- **Remove the instrumentation** once you have the origin.

## Finding which test or caller pollutes shared state

A bad value or artifact appears only sometimes (e.g. under the full suite but not in isolation) → the cause is usually order-dependent pollution from another caller. Bisect to find it: run the suite in halves, then quarters, narrowing to the first unit whose presence reproduces the pollution. Keep any bisection helper inside the host project, not as a dependency of this skill.
